...
contract TheAmazingTozziDuckMachine is ERC721, OwnableByERC721 {
    ...
    uint public constant BURN_WINDOW = 1 weeks;

    mapping (uint256 => address) public duckIdToWEBP;
    mapping (bytes32 => bool) public isCustomExisting;
    mapping (uint256 => uint) public customDuckHatchedTimes; 
    ...
    event DuckPriceUpdated(DuckType indexed duckType, address indexed who, uint256 price);
    
    event MaxCustomDucksUpdated(address indexed who, uint256 newMaxCustomDucks);

    event DuckMintingStatusUpdated(DuckType indexed duckType, address indexed who, bool isEnabled);

    event DuckMinted(DuckType indexed duckType, address indexed who, uint256 tokenId, uint256 price);

    event CustomDuckBurned(uint256 duckId, address admin, address owner, string reason);
    ...
    function flipTozziStatus() public onlyOwner {
        ...
        emit DuckMintingStatusUpdated(DuckType.Tozzi, msg.sender, tozziDucksEnabled);
    }

    function flipCustomStatus() public onlyOwner {
        customDucksEnabled = !customDucksEnabled;
        emit DuckMintingStatusUpdated(DuckType.Custom, msg.sender, tozziDucksEnabled);
    }

    function burnRenegadeDuck(uint256 duckId, string calldata reason) public onlyOwner {
        require(_exists(duckId), "Duck does not exist.");
        uint now = block.timestamp;
        require(now <= customDuckHatchedTimes[duckId] + BURN_WINDOW, "Burn window has passed");
    ...
    */ 
    function mintTozziDuck(uint256 duckId, string calldata webp, bytes32[] calldata merkleProof) public payable {        
        ...
        require(MerkleProof.verify(merkleProof, _MERKLE_ROOT, node),"invalid proof");
    ...
    function mintCustomDuck(string calldata webp) public payable {
        ...
        require(_customCounter + 1 <= maxCustomDucks, "Custom duck limit reached");
