...
const getRandomNoun = async () => {
  ...
  const [head, body, bill, hat, decor] = data.parts;
  const backgroundColor = getRandom(data.bgcolors);
  const parts = [head[0], body[0], bill[0], hat[0], decor[0]];
  ...
  const svgWithoutEndTag = parts.reduce((result, part) => {
    ...
    return result;
  }, `<svg width="400" height="400" viewBox="0 0 400 400" xmlns="http://www.w3.org/2000/svg" shape-rendering="crispEdges"><rect width="100%" height="100%" fill="#${backgroundColor}" />`);

  return `${svgWithoutEndTag}</svg>`;
};

const assembleDuck = async () => {
