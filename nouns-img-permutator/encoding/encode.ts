...
const updateBounds = (bounds: ImageBounds, lines: Lines, y: number) => {
  const { rects } = lines[y];
    if (!(rects[0].length === 40 && rects[0].colorIndex === 0) && bounds.top === 0) {
      bounds.top = y === 0 ? y : y - 1; // shift top bound to `y - 1` if > 0
    }
    if (bounds.top !== 0) {
      if ((rects[0].length === 40 && rects[0].colorIndex === 0) || y === 39) {
        if (bounds.bottom === 0) {
    ...
    lines[y].bounds = {
      left: rects[0].length,
      right: 40 - rects[rects.length - 1].length,
    };
};
...
const getEncodedImage = async (folder: string, file: string) => {
  ...
  for (let i = 39; i > bounds.bottom; i--) {
  ...
  if (lineLength) {
    ...
    if (lineLength === 1 && rect?.length === 40 && rect?.colorIndex === 0) {
      return '0x0000000000';
    }
  }
    ...
    const lineBuffer = Buffer.from(
      line.rects.flatMap(({ length, colorIndex }, i) => {
        ...
        if (i === line.rects.length - 1) {
          if (length > 40 - bounds.right) {
            return [length - (40 - bounds.right), colorIndex];
          } else if (length === 40 - bounds.right) {
            return [];
          }
        }
